    ...
    <div class="modal">
        <div class="form" @keyup.esc="cancelEvent">
            ...
            <div class="form-group">
                ...
                ref="name" placeholder="전화번호를 입력하세요"/>
            </div>
            <div class="form-group">
                <label>주소</label>
...
export default {
    ...
    computed : {
        btnText : function(){
            if (this.mod != 'update') return '추  가';
            else return '수  정';
        },
    headingText : function(){
        if (this.mod != 'update') return '새로운 연락처 추가';
        else return '연락처 변경';
        },
    },
    methods : {
        submitEvent : function(){
        if (this.mod == 'update') eventBus.$emit("updateSubmit", this.contact)
